<template>
  <div class="page-content">
    <div class="product-information">
      <el-form ref="form" :model="form" label-width="120px" :rules="rules">
        <div class="formWrap">
          ...
          <el-form-item label="游戏平台：" prop="gamePlatform">
            <el-select v-model="gamePlatform" placeholder="请选择游戏平台" multiple filterable clearable style="width: 100%">
              <el-option
                label="手机游戏"
                value="手机游戏"
              />
        ...
        <div class="formWrap">
          ...
          <div class="addForm">
            ...
            >
              <span slot-scope="{node, data}" class="custom-tree-node">
                <input :id="data.number" v-model="data.name" type="text">
                <span v-if="data.name === ''" class="el-tree-message">请输入名称</span>
                <span>
                  <el-button
                    class="addService"
      ...
      <div class="btn">
        <el-button type="primary" @click="addGame">发布游戏</el-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          this.$nextTick(() => {
            ...
            console.log(isError)
            console.log(isError[0].querySelector('input'))
            isError[0].querySelector('input[type="text"]').focus()
    ...
    // 添加服务器
    append(data) {
      const newChild = { number: serviceId++, isActive: true, name: '' }
      if (!data.children) {
